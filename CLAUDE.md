...
`sheets.js` splits `fetchTerritoryData` (download) from `parseTerritoryCsv` (column mapping) so the audit scripts can run against a local CSV, and it cache-busts the published CSV URL — Google serves `/pub?output=csv` with caching, which made edits to the sheet look like the app "wasn't updating".
